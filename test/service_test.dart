...
void main() {
  ...
  when(
    managerForService.writeCharacteristicForService(
      any,
      any,
      any,
      any,
  ...
  Service service = Service.fromJson(
    {"serviceId": 1, "serviceUuid": "testUuid"},
    peripheral,
    managerForService,
  );
  ...
  test("characteristics returns characteristics provided by manager", () async {
    //given
    when(managerForService.characteristicsForService(service)).thenAnswer(
      (_) => Future.value([
  ...
  test(
    ...
    () async {
      //given
      when(managerForService.descriptorsForService(service, "123")).thenAnswer(
        (_) => Future.value([
          createDescriptor(0),
          createDescriptor(1),
          createDescriptor(2),
  ...
  test("readCharacteristic returns expected characteristic", () async {
    ...
    //when
    var characteristic = await service.readCharacteristic(
      "123",
      transactionId: "a456",
    );
  ...
  test("readDescriptor returns expected descriptor", () async {
    ...
    //when
    var characteristic = await service.readDescriptor(
      "123",
      "456",
      transactionId: "a789",
    );
  ...
  test("monitorCharacteristic streams expected characteristic", () {
    ...
    //when
    var characteristicNotifications = service.monitorCharacteristic(
      "123",
      transactionId: "a456",
    );
